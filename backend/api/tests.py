...
class ViewSetTestCase(APITestCase):
    ...
    def test_post_data(self):
        grade = [{"courseCode": "ORGCHEM", "student": "2283853", "alphanum": "C2"}]
        response = self.client.post("/grades/", grade, format='json')
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)

    def test_data_stored(self):
        data = [{"courseCode": "INORG", "student": "1234567", "alphanum": "B2"},
                {"courseCode": "PHYS", "student": "2283653", "alphanum": "D2"}]
        serializer = GradeSerializer(data=data, many=True)
        if serializer.is_valid():
            serializer.save()
        grade = Grade.objects.filter(courseCode="INORG", student="1234567", alphanum="B2").exists()
        self.assertTrue(grade)

    def test_get_object(self):
        Grade.objects.get_or_create(courseCode="INORG", student="1234567", alphanum="B2")
    ...
    def test_duplicate_entries_not_created(self):
        ...
        grade = [{"courseCode": "PHYS", "student": "1234387", "alphanum": "E2"}]
        response = self.client.post("/grades/", grade, format='json')
